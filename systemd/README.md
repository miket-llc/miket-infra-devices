...
The systemd user service (`op-session.service`) and timer (`op-session.timer`) provide optional 1Password CLI session management on the akira control node. Use this for personal/interactive workflows only—automation now pulls secrets from Azure Key Vault into env files via `ansible/playbooks/secrets-sync.yml`, and should not depend on `op` sessions.
