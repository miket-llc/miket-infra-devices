...
smb_server: akira.pangolin-vega.ts.net
smb_username: mdt
smb_password: "{{ lookup(...) }}"
...
| Item | Path | Purpose |
|------|------|---------|
| Autofs master | `/etc/auto.master` | Registers the akira map |
| Autofs map | `/etc/auto.akira` | Defines share mount points |
| Credentials | `/root/.smbcredentials_akira` | SMB auth (mode 0600) |
| Mount base | `/mnt/akira/` | Parent directory for mounts |
...
| Servers (akira) | Bind mounts | It's the source of the shares |
...
# Manual mount test
sudo mount -t cifs //akira.pangolin-vega.ts.net/flux /mnt/test -o credentials=/root/.smbcredentials_akira
```
...
With autofs + soft mounts, operations should timeout gracefully. If you're still seeing hangs, check that the `soft,timeo=30` options are in `/etc/auto.akira`.
