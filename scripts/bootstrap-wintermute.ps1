<#
.SYNOPSIS
    Bootstrap script for Wintermute Windows workstation setup
.DESCRIPTION
    This script performs complete setup of Wintermute workstation for Ansible management:
    1. Creates mdt automation account
    2. Configures WinRM for Ansible
    3. Configures Tailscale with proper tags
    4. Verifies connectivity
    
    Run this script on wintermute as Administrator after cloning miket-infra-devices
#>

$ErrorActionPreference = "Stop"

Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host "   Wintermute Bootstrap Script" -ForegroundColor Cyan
Write-Host "   Windows 11 + RTX 4070 Super" -ForegroundColor Cyan
Write-Host "===============================================================" -ForegroundColor Cyan
...
# Step 1: Create mdt automation account
Write-Host "`n[Step 1/4] Creating mdt automation account..." -ForegroundColor Green
$accountScript = Join-Path $scriptDir "bootstrap-windows-automation-account.ps1"
if (Test-Path $accountScript) {
    & $accountScript
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Account creation may have failed - check output above"
    }
} else {
    Write-Warning "Account bootstrap script not found: $accountScript"
    Write-Host "You may need to create the mdt account manually or via Ansible" -ForegroundColor Yellow
}

# Step 2: Configure WinRM
Write-Host "`n[Step 2/4] Configuring WinRM for Ansible..." -ForegroundColor Green
...
Write-Host "`n[Step 3/4] Configuring Tailscale..." -ForegroundColor Green
$tailscaleScript = Join-Path $scriptDir "Setup-Tailscale.ps1"
if (Test-Path $tailscaleScript) {
    & $tailscaleScript -DeviceName "WINTERMUTE"
    if ($LASTEXITCODE -ne 0) {
...
# Step 4: Verify setup
Write-Host "`n[Step 4/4] Verifying setup..." -ForegroundColor Green

# Check mdt account
$mdtAccount = Get-LocalUser -Name "mdt" -ErrorAction SilentlyContinue
if ($mdtAccount) {
    Write-Host "[OK] mdt automation account exists" -ForegroundColor Green
} else {
    Write-Warning "[WARNING] mdt automation account not found"
}
...
Write-Host "   [SUCCESS] Wintermute Bootstrap Complete!" -ForegroundColor Green
Write-Host "===============================================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "       ~/miket-infra-devices/ansible/playbooks/windows-workstation.yml --limit wintermute" -ForegroundColor White
Write-Host ""
Write-Host "  3. Manual: Set up WSL2 with Ubuntu 24.04 and NVIDIA Container Toolkit" -ForegroundColor Yellow
Write-Host "     See: docs/runbooks/wintermute-setup.md" -ForegroundColor White
Write-Host ""
