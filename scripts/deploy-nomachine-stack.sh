...
display_plan() {
    ...
    echo "  • Linux (akira):       NoMachine server (VNC removed)"
    ...
    if $DEPLOY_SERVERS; then
        ...
        echo "  ✓ Install NoMachine on Linux (akira)"
...
display_testing_checklist() {
    ...
    echo ""
    echo "  [ ] From count-zero (macOS) to akira:"
    echo "      - Launch NoMachine"
    echo "      - Connect to akira.pangolin-vega.ts.net:4000"
    echo "      - Verify KDE desktop appears"
    ...
    echo ""
    echo "  [ ] From Windows (wintermute) to akira:"
    echo "      - Launch NoMachine client"
    echo "      - Connect to akira"
    ...
    echo "      - Verify changes appear on akira's physical filesystem"
