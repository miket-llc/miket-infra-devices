...
| DEV-004 | Add CI lint + smoke tests for mounts/remote access playbooks | Codex-PD-002 | None | 🚧 In Progress | Windows smoke playbook added; wire into CI with ansible-lint/check-mode next |
...
| DEV-015 | Execute multi-source reconciliation into `/space/mike` (count-zero + M365 + wintermute) | Codex-CA-001 | Reconciliation guidance + script | 🚧 In Progress | Run `reconcile-multi-source-transfers.sh`; confirm class/duplicate summary before promotion, keep manifests/conflict backups in run folder |
