<#
.SYNOPSIS
    Quick fix for MagicDNS on Wintermute (Windows)
.DESCRIPTION
...
Write-Host "`n🔧 MagicDNS Remediation Script for Wintermute" -ForegroundColor Cyan
...
try {
    ...
    $currentTags = if ($status.Self.Tags -and $status.Self.Tags.Count -gt 0) {
        ...
        # Fallback to default tags for wintermute
